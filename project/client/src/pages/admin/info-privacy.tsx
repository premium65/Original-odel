...
export default function AdminInfoPrivacy() {
  ...
  useEffect(() => {
    const privacyPage = pages.find(p => p.type === "privacy");
    if (privacyPage) {
      setContent({
        title: privacyPage.title || "Privacy Policy",
        content: privacyPage.content || "",
        isActive: privacyPage.isActive ?? true
      });
    }
  }, [pages]);
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ title: "Success", description: "Privacy Policy saved successfully" });
    },
    onError: () => {
      toast({ title: "Error", description: "Failed to save Privacy Policy", variant: "destructive" });
    },
  });
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[#9ca3af] mt-1">Edit your Privacy Policy page</p>
        </div>
