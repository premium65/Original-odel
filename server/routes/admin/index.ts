...
async function requireAdmin(req: Request, res: Response, next: NextFunction) {
    try {
        ...
        // Normalize session userId (could be string or number)
        const sessionUserId = String(req.session.userId);
        console.log("[ADMIN_AUTH_MIDDLEWARE] Session userId:", sessionUserId, "Type:", typeof req.session.userId);
        ...
        if (user) {
            if (!user.isAdmin) {
                console.log("[ADMIN_AUTH_MIDDLEWARE] User found but not admin:", sessionUserId);
                return res.status(403).json({ error: "Admin access required" });
            }
            console.log("[ADMIN_AUTH_MIDDLEWARE] Admin authenticated from PostgreSQL:", user.username);
            return next();
        }
            ...
            try {
                ...
                if (mongoUser) {
                    if (!mongoUser.isAdmin) {
                        console.log("[ADMIN_AUTH_MIDDLEWARE] MongoDB user found but not admin:", sessionUserId);
                        return res.status(403).json({ error: "Admin access required" });
                    }
                    console.log("[ADMIN_AUTH_MIDDLEWARE] Admin authenticated from MongoDB:", mongoUser.username);
                    return next();
                }
            } catch (err) {
        ...
        if (memUser) {
            if (!memUser.isAdmin) {
                console.log("[ADMIN_AUTH_MIDDLEWARE] In-memory user found but not admin:", sessionUserId);
                return res.status(403).json({ error: "Admin access required" });
            }
            console.log("[ADMIN_AUTH_MIDDLEWARE] Admin authenticated from in-memory:", memUser.username);
            return next();
        }

        console.log("[ADMIN_AUTH_MIDDLEWARE] User not found in any storage:", sessionUserId);
        return res.status(403).json({ error: "Admin access required" });
    } catch (error) {
